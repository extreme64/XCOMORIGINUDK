/**
 * Класс пуль.
 * Используется для видимых летящих пуль, ракет, гранат и т.д.
 */
class X_COM_Projectile extends UDKProjectile
...
var(Projectile) const SoundCue	AmbientSound;		// The sound that is played looped.
var(Projectile) const SoundCue	ExplosionSound;		// The sound that is played when it explodes
...
/** Effects Template */
var(Projectile) const ParticleSystem ProjFlightTemplate;
var(Projectile) const ParticleSystem ProjExplosionTemplate;
...
simulated function SetDamageParams( int aNewDamage, class<DamageType> aDamageType)
{
	Damage = aNewDamage;
...
simulated event PostBeginPlay()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		bWideCheck = bWideCheck || ((CheckRadius > 0) && (Instigator != None) && (Instigator.Controller != None));
	}
...
simulated function bool EffectIsRelevant(vector SpawnLocation, bool bForceDedicated, optional float CullDistance=5000.0, optional float HiddenCullDistance=350.0 )
{
	...
	return Super.EffectIsRelevant(SpawnLocation, bForceDedicated, CullDistance, HiddenCullDistance);
}
...
simulated function SpawnExplosionEffects(vector HitLocation, vector HitNormal)
{
	...
	if (WorldInfo.NetMode != NM_DedicatedServer)
	{
		...
		{
			EffectAttachActor = (bAttachExplosionToVehicles || (X_COM_Vehicle(ImpactedActor) == None)) ? ImpactedActor : None;
...
event Actor GetHomingTarget(X_COM_Projectile Seeker, Controller InstigatedBy) { return self; }
...
defaultproperties
{
	...
	bNetTemporary = TRUE // - This is set true as projectiles only need to be replicated initially, and then torn off. 
	bGameRelevant = TRUE // - This is set true as projectiles are always relevant to the game. 
	bReplicateInstigator = TRUE // - This is set true as the projectile Instigator variable should be replicated. 
	NetPriority = 2.5 // - This is set to 2.5 to have a higher network priority than other actors. However, in this case it only means that the initial replication will be done as quickly as possible. 
	Role = ROLE_Authority
	RemoteRole = ROLE_SimulatedProxy  // - This is set to ROLE_SimulatedProxy because projectiles should be simulated on the clients.
