 ...
 */
class X_COM_GameViewportClient extends UDKGameViewportClient
	config(Game);
...
function DrawTransition(Canvas Canvas)
{
	...
	local class<X_COM_GameInfo> GameClass;
	...
	if (Outer.TransitionType == TT_Loading)
	{
		...
		GameClass = class<X_COM_GameInfo>(FindObject(Outer.TransitionGameType, class'Class'));
		Desc = "";
		...
		if (GameClass == none)
		{
			...
			if(Pos != -1)
			{
				...
				if(Pos != -1)
				{
					...
					GameClass = class<X_COM_GameInfo>(FindObject(ParseStr, class'Class'));
		...
		if( bAllowHints )
		{
			...
			HintMessage = LoadRandomLocalizedHintMessage( string( class'X_COM_GameInfo'.Name ), GameClassName);
	...
	{
		Canvas.Font = class'X_COM_HUD'.static.GetFontSizeIndex(3);
		Canvas.SetPos(0, 0);
...
function NotifyConnectionError(EProgressMessageType MessageType, optional string Message=Localize("Errors", "ConnectionFailed", "Engine"), optional string Title=Localize("Errors", "ConnectionFailed_Title", "Engine") )
{
	...
	//class'X_COM_PlayerController'.static.SetFrontEndErrorMessage(Title, Message);

	// Start quitting to the main menu
	if (X_COM_PlayerController(Outer.GamePlayers[0].Actor) != None)
	{
		//X_COM_PlayerController(Outer.GamePlayers[0].Actor).QuitToMainMenu();
	}
	else
	{
